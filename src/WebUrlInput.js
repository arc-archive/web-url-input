...
/** @typedef {import('@advanced-rest-client/arc-models').UrlHistoryModel} UrlHistoryModel */
/**
 ...
 */
export class WebUrlInput extends ArcOverlayMixin(LitElement) {
  static get styles() {
    return css`
      :host {
  ...
  render() {
    const { _autocompleteTarget, value = '', compatibility, outlined } = this;
    const actionDisabled = !value;
    return html` <div class="inputs">
        <div class="listbox-wrapper">
          ...
          <anypoint-autocomplete
            ...
            ?compatibility="${compatibility}"
            @opened-changed="${this._suggestionsOpenedHandler}"
          ></anypoint-autocomplete>
        </div>
        <anypoint-button
          ?compatibility="${compatibility}"
  ...
  set suggestionsOpened(value) {
    ...
    this.dispatchEvent(
      new CustomEvent('suggestionsopened-changed', {
        composed: true,
        detail: {
          value,
        },
  ...
  async _makeQuery(q) {
    ...
    try {
      ...
      const items = result.map((item) => item.url);
      this._autocomplete.source = items;
    } catch (e) {
